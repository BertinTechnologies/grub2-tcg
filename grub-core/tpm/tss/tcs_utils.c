...
LoadBlob(UINT64 *offset, UINT32 size, BYTE *container, BYTE *object)
{
	...
	if (container)
		grub_memcpy(&container[*offset], object, size);
	(*offset) += (UINT64) size;
}
...
UnloadBlob(UINT64 *offset, UINT32 size, BYTE *container, BYTE *object)
{
	...
	if (object)
		grub_memcpy(object, &container[*offset], size);
	(*offset) += (UINT64) size;
}
...
UnloadBlob_KEY_PARMS(UINT64 *offset, BYTE *blob, TCPA_KEY_PARMS *keyParms)
{
	...
	if (keyParms->parmSize == 0)
		...
		keyParms->parms = grub_malloc(keyParms->parmSize);
		if (keyParms->parms == NULL) {
			LogError("grub_malloc of %u bytes failed.", keyParms->parmSize);
			keyParms->parmSize = 0;
...
UnloadBlob_CERTIFY_INFO(UINT64 *offset, BYTE *blob, TCPA_CERTIFY_INFO *certify)
{
	...
	if (certify->PCRInfoSize > 0) {
		certify->PCRInfo = (BYTE *)grub_malloc(certify->PCRInfoSize);
		if (certify->PCRInfo == NULL) {
			LogError("grub_malloc of %u bytes failed.", certify->PCRInfoSize);
			certify->PCRInfoSize = 0;
			grub_free(certify->algorithmParms.parms);
...
UnloadBlob_KEY_HANDLE_LIST(UINT64 *offset, BYTE *blob, TCPA_KEY_HANDLE_LIST *list)
{
	...
	list->handle = grub_malloc(list->loaded * sizeof (UINT32));
        if (list->handle == NULL) {
		LogError("grub_malloc of %zd bytes failed.", list->loaded * sizeof (UINT32));
		list->loaded = 0;
                return TCSERR(TSS_E_OUTOFMEMORY);
        }
