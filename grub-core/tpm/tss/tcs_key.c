...
UnloadBlob_STORE_PUBKEY(UINT64 *offset, BYTE *blob, TCPA_STORE_PUBKEY *store)
{
	...
	if (store->keyLength == 0) {
		...
		store->key = (BYTE *)grub_malloc(store->keyLength);
		if (store->key == NULL) {
			LogError("grub_malloc of %u bytes failed.", store->keyLength);
			store->keyLength = 0;
...
UnloadBlob_TSS_KEY(UINT64 *offset, BYTE *blob, TSS_KEY *key)
{
	...
	if (key->PCRInfoSize == 0)
		...
		key->PCRInfo = grub_malloc(key->PCRInfoSize);
		if (key->PCRInfo == NULL) {
			LogError("grub_malloc of %u bytes failed.", key->PCRInfoSize);
			key->PCRInfoSize = 0;
			grub_free(key->algorithmParms.parms);
	...
	if (key->encSize == 0)
		...
		key->encData = (BYTE *)grub_malloc(key->encSize);
		if (key->encData == NULL) {
			LogError("grub_malloc of %d bytes failed.", key->encSize);
			key->encSize = 0;
			grub_free(key->algorithmParms.parms);
			key->algorithmParms.parms = NULL;
			key->algorithmParms.parmSize = 0;
