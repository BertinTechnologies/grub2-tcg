...
#define TSS_BUILD_AUDIT 1
#define TSS_BUILD_CMK 1
#define TSS_BUILD_COUNTER 1
#define TSS_BUILD_DAA 1
#define TSS_BUILD_DELEGATION 1
#define TSS_BUILD_QUOTE 1
#define TSS_BUILD_NV 1
#define TSS_BUILD_TICK 1
#define TSS_BUILD_TRANSPORT 1
#define TSS_BUILD_TSS12 1
...
tpm_rsp_parse(TPM_COMMAND_CODE ordinal, BYTE *b, UINT32 len, ...)
{
	...
	case TPM_ORD_ExecuteTransport:
	{
		...
		*len1 = offset2 - offset1;
		if (*len1) {
			if ((*blob1 = grub_malloc(*len1)) == NULL) {
				LogError("grub_malloc of %u bytes failed", *len1);
	...
	case TPM_ORD_TickStampBlob:
	{
		...
		*len1 = (UINT32)offset2 - offset1;
		...
		if ((*blob2 = grub_malloc(*len2)) == NULL) {
			LogError("grub_malloc of %u bytes failed", *len2);
			grub_free(*blob1);
			*blob1 = NULL;
			*len1 = 0;
			*len2 = 0;
	...
	case TPM_ORD_Quote2:
	{
		...
		*len1 = offset2 - offset1;
		...
		/* sig */
		if ((*blob3 = grub_malloc(*len3)) == NULL) {
			LogError("grub_malloc of %u bytes failed", *len3);
			grub_free(*blob1);
			*blob1 = NULL;
			if (*len2 > 0){
				grub_free(*blob2);
	...
	case TPM_ORD_ActivateIdentity:
	{
		...
		if ((*blob1 = grub_malloc(offset2)) == NULL) {
			LogError("grub_malloc of %zd bytes failed", (grub_size_t)offset2);
			return TCSERR(TSS_E_OUTOFMEMORY);
		}
		*len1 = offset2;
		UnloadBlob(&offset1, *len1, b, *blob1);
	...
	case TPM_ORD_MakeIdentity:
	{
		...
		if ((*blob1 = grub_malloc(offset1)) == NULL) {
			LogError("grub_malloc of %zd bytes failed", (grub_size_t)offset1);
			return TCSERR(TSS_E_OUTOFMEMORY);
		}
		*len1 = offset1;
	...
	case TPM_ORD_CMK_ConvertMigration:
	{
		...
		if ((*data = grub_malloc(*data_len)) == NULL) {
			LogError("grub_malloc of %u bytes failed", *data_len);
			return TCSERR(TSS_E_OUTOFMEMORY);
		}
	...
	* return: UINT32 *, BYTE**, 1 optional AUTH*/
	...
	case TPM_ORD_CMK_CreateKey:
	{
		...
		offset1 = TSS_TPM_TXBLOB_HDR_LEN;
		offset2 -= offset1;
		if ((*data = grub_malloc((grub_size_t)offset2)) == NULL) {
			LogError("grub_malloc of %zd bytes failed", (grub_size_t)offset2);
			return TCSERR(TSS_E_OUTOFMEMORY);
		}
