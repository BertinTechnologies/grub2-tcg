 ...
 * \brief Call the TCG function ASM_TCG_StatusCheck.
 *        The result is stored in a TCG_STATUSCHECK structure.	
 *
 * \param status Pointer to a TCG_STATUSCHECK structure where result will be stored.
 * \return -1 if the tpm is not present or if the int 0x1A fails ;
 *          0 if success (status->code must be checked)
 */
...
grub_uint32_t TCG_HashLogExtendEvent(grub_uint8_t * addr,
				     grub_uint32_t size,
				     grub_uint32_t pcrIndex,
				     grub_uint32_t eventType,
				     char * eData,
				     grub_uint32_t eDataSize) {
  struct grub_bios_int_registers 	regs;
  ...
  if(GRUB_MEMORY_MACHINE_SCRATCH_SIZE <
     sizeof(TCG_HashLogExtendEvent_IPB) +
     sizeof(TCG_HashLogExtendEvent_OPB) +
     sizeof(TCG_PCClientPCREventStruct) + eDataSize) {
    return TCGERR_ERROR_REALMEM;
  }

  ipb = (TCG_HashLogExtendEvent_IPB *) GRUB_MEMORY_MACHINE_SCRATCH_ADDR;
  opb = (TCG_HashLogExtendEvent_OPB *) (GRUB_MEMORY_MACHINE_SCRATCH_ADDR +
					sizeof(TCG_HashLogExtendEvent_IPB));
  event = (TCG_PCClientPCREventStruct *) (GRUB_MEMORY_MACHINE_SCRATCH_ADDR +
					  sizeof(TCG_HashLogExtendEvent_IPB) +
					  sizeof(TCG_HashLogExtendEvent_OPB));
  eventData = (BYTE *) (GRUB_MEMORY_MACHINE_SCRATCH_ADDR +
			sizeof(TCG_HashLogExtendEvent_IPB) +
			sizeof(TCG_HashLogExtendEvent_OPB) + sizeof(TCG_PCClientPCREventStruct));
  ...
  grub_memset((grub_uint8_t *) GRUB_MEMORY_MACHINE_SCRATCH_ADDR, 0,
	      sizeof(TCG_HashLogExtendEvent_IPB) +
	      sizeof(TCG_HashLogExtendEvent_OPB) +
	      sizeof(TCG_PCClientPCREventStruct) + eDataSize);
...
grub_uint32_t TCG_CompactHashLogExtendEvent(grub_uint8_t * addr,
					    grub_uint32_t size,
					    grub_uint32_t pcrIndex,
					    grub_uint32_t eventType) {
  struct grub_bios_int_registers	regs;
...
grub_uint32_t TCG_PassthroughToTPM(BYTE * txBlob) {
  ...
  opb = (TCG_PassThroughToTPM_OPB *) (GRUB_MEMORY_MACHINE_SCRATCH_ADDR +
				      sizeof(TCG_PassThroughToTPM_IPB));
