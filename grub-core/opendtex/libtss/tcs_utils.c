...
LoadBlob_Header(UINT16 tag, UINT32 paramSize, UINT32 ordinal, BYTE * blob)
{
	...
	LogData((char *)"Header Tag:", tag);
	UINT32ToArray(paramSize, &blob[2]);
	LogData((char *)"Header ParamSize:", paramSize);
	UINT32ToArray(ordinal, &blob[6]);
	LogData((char *)"Header Ordinal:", ordinal);
#if 0
...
LogUnloadBlob_Header(BYTE * blob, UINT32 * size, char *file, int line)
{
	...
	LogData((char *)"UnloadBlob_Tag:", (temp));
	*size = Decode_UINT32(&blob[2]);
	LogData((char *)"UnloadBlob_Header, size:", *size);
	LogData((char *)"UnloadBlob_Header, returnCode:", Decode_UINT32(&blob[6]));
