...
#include <tboot/io.h>
#include <opendtex/vga_switch_mode.h> //init_graph_vga and bitmap definition
...
/**
 * Initilization of the color palette.
...
int set_palette (BYTE * palette) {
	...
	//Tell VGA palette data is coming
	outb(0x03c8, 0);

	for (i=0; i<1020; i+=4) {
  		outb(0x03c9, palette[i+2] >> 2);
  		outb(0x03c9, palette[i+1] >> 2);
  		outb(0x03c9, palette[i] >> 2);

  		//Switch the 4th byte (which is always 0)
  		if (palette[i+3] != 0) {
  			printk("Error with palette initialization at byte %d: %x %x %x %x\n", i, palette[i], palette[i+1], palette[i+2], palette[i+3]);
  			return 0;
  		}
	}
	return 1;
