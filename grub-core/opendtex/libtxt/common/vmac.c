...
{   int ii; uint64_t th, tl;                                                  \
    rh = rl = 0;                                                             \
    for (ii = 0; ii < nw; ii+= 2) {                                             \
        MUL64(th,tl,get64PE((mp)+ii  )+(kp)[ii  ],get64PE((mp)+ii+1)+(kp)[ii+1]);\
        ADD128(rh,rl,th,tl);                                                 \
    }                                                                        \
}
...
{   int ii; uint64_t th, tl;                                                  \
    ...
    for (ii = 0; ii < nw; ii+= 2) {                                             \
        MUL64(th,tl,get64PE((mp)+ii  )+(kp)[ii  ],get64PE((mp)+ii+1)+(kp)[ii+1]);\
        ADD128(rh,rl,th,tl);                                                 \
        MUL64(th,tl,get64PE((mp)+ii  )+(kp)[ii+2],get64PE((mp)+ii+1)+(kp)[ii+3]);\
...
{   int ii; uint64_t th, tl;                                                  \
    ...
    for (ii = 0; ii < nw; ii+= 8) {                                             \
        MUL64(th,tl,get64PE((mp)+ii  )+(kp)[ii  ],get64PE((mp)+ii+1)+(kp)[ii+1]);\
        ADD128(rh,rl,th,tl);                                                 \
        MUL64(th,tl,get64PE((mp)+ii+2)+(kp)[ii+2],get64PE((mp)+ii+3)+(kp)[ii+3]);\
        ADD128(rh,rl,th,tl);                                                 \
        MUL64(th,tl,get64PE((mp)+ii+4)+(kp)[ii+4],get64PE((mp)+ii+5)+(kp)[ii+5]);\
        ADD128(rh,rl,th,tl);                                                 \
        MUL64(th,tl,get64PE((mp)+ii+6)+(kp)[ii+6],get64PE((mp)+ii+7)+(kp)[ii+7]);\
        ADD128(rh,rl,th,tl);                                                 \
    }                                                                        \
}
...
{   int ii; uint64_t th, tl;                                                  \
    ...
    for (ii = 0; ii < nw; ii+= 8) {                                             \
        ...
        MUL64(th,tl,get64PE((mp)+ii+2)+(kp)[ii+2],get64PE((mp)+ii+3)+(kp)[ii+3]);\
        ADD128(rh,rl,th,tl);                                                 \
        MUL64(th,tl,get64PE((mp)+ii+2)+(kp)[ii+4],get64PE((mp)+ii+3)+(kp)[ii+5]);\
        ADD128(rh1,rl1,th,tl);                                               \
        MUL64(th,tl,get64PE((mp)+ii+4)+(kp)[ii+4],get64PE((mp)+ii+5)+(kp)[ii+5]);\
        ADD128(rh,rl,th,tl);                                                 \
        MUL64(th,tl,get64PE((mp)+ii+4)+(kp)[ii+6],get64PE((mp)+ii+5)+(kp)[ii+7]);\
        ADD128(rh1,rl1,th,tl);                                               \
        MUL64(th,tl,get64PE((mp)+ii+6)+(kp)[ii+6],get64PE((mp)+ii+7)+(kp)[ii+7]);\
        ADD128(rh,rl,th,tl);                                                 \
        MUL64(th,tl,get64PE((mp)+ii+6)+(kp)[ii+8],get64PE((mp)+ii+7)+(kp)[ii+9]);\
        ADD128(rh1,rl1,th,tl);                                               \
    }                                                                        \
}
#endif
                  ...
                  vmac_ctx_t    *ctx)
{
    ...
    mptr = (uint64_t *)m;
    j = mbytes / VMAC_NHBYTES;  /* Must be non-zero */
